/* تَسَابِيح ∞ v21 — Recovery & Fitness View */
  ...
  el.innerHTML = `
    ...
    <div class="rec-hero">
      <div class="rec-streak-ring-wrap">
        <svg viewBox="0 0 120 120" class="rec-ring-svg">
          ...
          <circle cx="60" cy="60" r="52" class="ring-fill rec-ring"
            ...
            stroke-dashoffset="${2*Math.PI*52*(1-Math.min(rec.streak,90)/90)}"/>
        </svg>
      ...
      <div class="rec-meta">
        ...
        <div class="rec-total">مجموع أيامك النظيفة: <strong>${aN(rec.totalCleanDays)}</strong></div>
        <div class="rec-best">أفضل سلسلة: ${aN(rec.bestStreak)} يوم</div>
      </div>
    </div>

    <!-- ── Flatline Warning ── -->
    ${inFL ? `
    <div class="flatline-banner">
      <div class="flatline-title">⚠️ مرحلة الـ Flatline (يوم ${aN(rec.streak)})</div>
      <div class="flatline-msg">الشعور بالفتور الآن طبيعي 100٪ — دماغك يبني مستقبلات جديدة</div>
    </div>` : ''}

    <!-- ── Next Milestone ── -->
    ${next ? `
    <div class="rec-milestone">
      ${next.icon} هدفك القادم: يوم ${aN(next.day)} — ${next.label}
      (${aN(next.day - rec.streak)} يوم متبقي)
    </div>` : ''}

    <!-- ── Action Buttons ── -->
    <div class="rec-actions">
      <button class="btn-rec-action btn-checkin ${checkedIn?'done':''}" id="btn-rec-checkin">
        ${checkedIn ? '✅ سجّلت يومك' : '✅ أنا نقي اليوم'}
      </button>
      <button class="btn-rec-action btn-freeze" id="btn-rec-freeze">
        🧊 Freeze (${aN(rec.freezesLeft || 0)})
      </button>
      <button class="btn-rec-action btn-sos" id="btn-rec-sos">🚨 إنقاذ طارئ</button>
    </div>

    <!-- ── Brain Recovery Dashboard ── -->
    <div class="brain-section">
      <div class="brain-title">🧬 تعافي الدماغ — علم الأعصاب</div>
        ...
        ${BRAIN_METRICS.map(m => `
          <div class="brain-metric">
            ...
            <div class="bm-bar-wrap">
              <div class="bm-bar" style="width:${Math.round(brain[m.key])}%"></div>
            </div>
            <div class="bm-full-day">يكتمل يوم ${aN(m.fullDay)}</div>
          </div>
        `).join('')}
      </div>
      <div class="brain-boost-note">
        💪 التمرين يُسرّع تعافي D2 +6٪ | 🕌 الصلاة المنتظمة +3٪
      </div>
    </div>
    ...
    <div class="fit-section" id="fit-section">
      ...
      <div class="fit-exercise" id="fit-push-card">
        <div class="fit-ex-header">
          ...
          <span class="fit-ex-name">ضغط (Push-ups)</span>
        ...
        <div class="fit-btns">
          ...
          <button class="btn-fit-manual" data-type="push">إدخال يدوي</button>
        </div>
      </div>
      ...
      <div class="fit-exercise" id="fit-squat-card">
        <div class="fit-ex-header">
          <span class="fit-ex-icon">🦵</span>
          <span class="fit-ex-name">قرفصاء (Squats)</span>
        ...
        <div class="fit-btns">
          ...
          <button class="btn-fit-manual" data-type="squat">إدخال يدوي</button>
        </div>
      </div>
...
function _refreshFitness() {
  ...
  if (rt) rt.querySelector('span').textContent = '⏱ الراحة: ' + fmtCountdown(fit.restSecondsLeft);
}
